...
function render(){
 ...
 poem.lines.forEach((x,i)=>{const img=new Image();img.onload=()=>{$$('.scene')[i].style.backgroundImage=`url("${x.image}")`};img.src=x.image});
 musicAudio=new Audio(poem.music);musicAudio.loop=true;musicAudio.volume=.22; update();
}
...
function speak(cb){if(voiceAudio){voiceAudio.pause();voiceAudio=null}const a=new Audio(poem.lines[current].audio);voiceAudio=a;let f=false;a.onended=()=>cb&&cb();a.onerror=()=>{if(!f){f=true;fallback(cb)}};a.play().catch(()=>fallback(cb))}
